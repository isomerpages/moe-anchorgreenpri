---
title: "Stall No 6: Malay Rice Food Stall"
permalink: /useful-links/resources-for-parents/canteen-menu/stall-no-6-malay-rice-food-stall/
description: ""
variant: markdown
---
Stall No. 6: Malay Rice Food Stall
----------------------------------

#### **Set Meal Concept:&nbsp;Rice + Vegetables + Meat + Fruit**

#### **Serving Sizes:&nbsp;Medium ($2.00)&nbsp;Large ($2.50)**

### Main Meals

<table style="border: 1px solid #2A2A2A; border-collapse: collapse; font-family: Prompt, sans-serif; font-size: 16px; width: 100%;">
  <tbody>
    <tr>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;"><b>Set</b></td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;"><b>Monday</b></td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;"><b>Tuesday</b></td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;"><b>Wednesday</b></td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;"><b>Thursday</b></td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;"><b>Friday</b></td>
    </tr>
    <tr>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">A</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Chicken Rice Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Chicken Rice Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Chicken Rice Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Chicken Rice Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Chicken Rice Set</td>
    </tr>
    <tr>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">B</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Mayo Cutlet Chicken Rice</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Mayo Cutlet Chicken Rice</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Mayo Cutlet Chicken Rice</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Mayo Cutlet Chicken Rice</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Mayo Cutlet Chicken Rice</td>
    </tr>
    <tr>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">C</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Roti Kirai/ Jala Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Buttermilk Chicken Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Prata with Curry</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Prata with Curry</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Roti Kirai/ Jala Set</td>
    </tr>
    <tr>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">D</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Tom Yum Chicken Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Prata with Curry</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Curry Chicken Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Curry Chicken Set</td>
      <td style="border: 1px solid #2A2A2A; text-align: center; padding: 2px;">Japanese Curry Chicken Set</td>
    </tr>
  </tbody>
</table>